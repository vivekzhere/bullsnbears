...
require_once("includes/config.php");
	...
	function Sell($id, $symbol, $amount, $stock_data, $skey) {
		...
		if ($p) {
			...
			if ($p) {
				$p = $mysqli->query("INSERT INTO `history` ( `p_id`, `t_type`, `symbol`, `amount`, `value` ) VALUES ( '{$id}', 'S', '{$symbol}', '{$amount}', '{$stock_data['value']}' )"); 
				if (!$p) $mysqli->rollback();
				else {
					$mysqli->commit();
					$err_flag = FALSE;
	...
	function Short($id, $symbol, $amount, $stock_data, $skey) {
		...
		$err_flag = TRUE;
		$mysqli->autocommit(FALSE);
		$p = $mysqli->query("INSERT INTO `short_sell` VALUES( '{$_SESSION['id']}', '{$symbol}', '{$amount}', '{$stock_data['value']}' ) ON DUPLICATE KEY UPDATE `val` = ((`val` * `amount`) + ".($amount * $stock_data['value'])." ) / (`amount` + ".$amount." ), `amount` = `amount` + ".$amount);
		if ($p) {
			$p = $mysqli->query("UPDATE `player` SET `rank` = 1, `liq_cash` = `liq_cash` - ".round($amount * $stock_data['value'] * 0.002).", `short_val` = `short_val` + ".round($amount * $stock_data['value'] )." WHERE `id` = '{$_SESSION['id']}'");
			if ($p) {
				$p = $mysqli->query("INSERT INTO `history` ( `p_id`, `t_type`, `symbol`, `amount`, `value` ) VALUES ( '{$_SESSION['id']}', 'SS', '{$symbol}', '{$amount}', '{$stock_data['value']}' )"); 
				if (!$p) $mysqli->rollback();
				else {
					$mysqli->commit();
					$err_flag = FALSE;
	...
	function Cover($id, $symbol, $amount, $stock_data, $skey) {
		...
		if ($x) {
			$x = $x->fetch_assoc();
			if ($amount != $stock_data['shorted_amount']) $p = $mysqli->query("UPDATE `short_sell` SET `val` = ((`val` * `amount`) - ".($amount * $stock_data['value'])." ) / (`amount` - ".$amount." ), `amount` = `amount` - ".$amount." WHERE `id` = '{$_SESSION['id']}' AND `symbol` = '{$symbol}'");
			else $p = $mysqli->query("DELETE FROM `short_sell` WHERE `id` = '{$_SESSION['id']}' AND symbol = '{$symbol}'"); 
			if ($p) {
				$p = $mysqli->query("UPDATE `player` SET `liq_cash` = `liq_cash` + ".round($x['old'] - $amount * $stock_data['value'] * 0.998).", `short_val` = Case When (`short_val` - ".ceil($amount * $stock_data['value']).") < 0 THEN 0 ELSE (`short_val` - ".ceil($amount * $stock_data['value']).") END WHERE `id` = '{$_SESSION['id']}'");
				if ($p) {
					$p = $mysqli->query("INSERT INTO `history` ( `p_id`, `t_type`, `symbol`, `amount`, `value` ) VALUES ( '{$_SESSION['id']}', 'C', '{$symbol}', '{$amount}', '{$stock_data['value']}' )"); 
					if (!$p) $mysqli->rollback();
					else {
						$mysqli->commit();
						$err_flag = FALSE;
