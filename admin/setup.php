...
require_once("../includes/connection.php");
	...
	if ($sqlid == '') {
		...
		if (!$mysqli->query($sql)) {
			...
			$mysqli->query($sql) or die($sql.'<br />'.$mysqli->error);
			...
			$sql = "CREATE TABLE IF NOT EXISTS `notice` (`message` varchar(1000) NOT NULL, `player_id` varchar(30) NOT NULL DEFAULT 'g',
					`time_stamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=latin1";
			$mysqli->query($sql) or die($sql.'<br />'.$mysqli->error);
